...
/* copied and modified from upstream forstdin:
 * no checking for EOF (just return 0 on an empty file)
 * use skagetlnsep_loose to ensure final line (possibly missing null) is called
 */
...
int main (int argc, char const **argv)
{
  ...
  for (;;)
  {
    ...
    if (delimlen)
    {
      int r = skagetlnsep_loose(buffer_0, &value, delim, delimlen) ;
      if (!r) break ;
      else if (r < 0)
      {
